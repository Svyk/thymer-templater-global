...
**v2.49.1:** Journal duplicate reconciliation re-fetches line handles before every move attempt and falls through compatible SDK call shapes when Thymer rejects one. A failed move defers only that template group, never authorizes deletion, and retries on the next convergence trigger. For a guarded live check on the active Journal page, call `window.__TEMPLATER_MOVE_PROBE({lineGuid, targetHeadingGuid})`; it moves one line, returns the successful shape plus all attempts, and never deletes content.
