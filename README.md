...
**v2.48.2:** fixes stale-editor failures when inserting from the `;;` popup and adds Ctrl+O / Cmd+O to toggle a dry-render preview beside (or below) the popup. The popup remains fully opaque and scoped to **Type=Snippet** templates by default; choose **all** in Templater settings for regular templates too.
