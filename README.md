...
**v2.49.0:** Journal auto-apply is exactly-once across tabs, Desktop, and independent browsers. Same-browser opens use a page-day Web Lock plus a lease written before the first await. Every generated Journal line is marked immediately through `setMetaProperties`, and the deploy-time `window.__templater.RECONCILE()` capability converges old, marker-less, stale, and CLI-created duplicate sections by keeping the smallest heading GUID, preserving user lines through GUID-stable moves, and deleting only proven template twins. Reconciliation runs before apply, after navigation convergence, on synthetic Journal line creation/restoration, and once for today at boot.
