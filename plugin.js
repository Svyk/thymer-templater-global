    ...
    try {
      ...
      try {
        ...
        for (const f of ((cfg && cfg.fields) || [])) {
          ...
          if (v != null && String(v).trim()) propsTxt += f.label + (f.type === 'choice' && f.choices ? ' (choice of: ' + f.choices.map(c => c.label || c).join(', ') + ')' : '') + ': ' + String(v).slice(0, 120) + '\n';
        }
      } catch (e) {}
